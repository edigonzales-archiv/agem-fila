...
public class App {
	...
	public static void main(String[] args) {
		...
		try {
	        ...
	        params.put("host", dbhost);
	        params.put("port", Integer.valueOf(dbport));
	        params.put("database", dbdatabase);
	        params.put("schema", dbschema);
	        params.put("user", dbusr);
	        params.put("passwd", dbpwd);

	        DataStore dataStore = DataStoreFinder.getDataStore(params);
	        dataStoreRepo.register("RoadAxis", dataStore);
			...
			// 
			// setup graph
			BasicLineGraphGenerator basicLineGen = new BasicLineGraphGenerator();			
			
			for (LineSegment seg : segments) {
				basicLineGen.add(seg);
			}
			
			Graph graph = basicLineGen.getGraph();
			...
			com.vividsolutions.jts.geom.Coordinate coord = (Coordinate) destination.getObject();
			log.info(String.valueOf(coord.toString()));
			destinations.add(destination);
			
			log.info("**");
			for (Node dest : destinations) {
				log.info(String.valueOf(dest.getID()));
